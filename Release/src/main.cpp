...
void init(void)
{
	...
	shaderprogram = initShaders("./src/shaders/simple - Copy.vert", "./src/shaders/simple - Copy.frag"); // Create and start shader program
	...
	std::cout << vao << vbo[0] << vbo[1] << std::endl;
...
void draw(SDL_Window* window, Model* m)
{
	...
	////glDrawArrays(GL_TRIANGLES, 0, 3); // draw the pyramid

	//std::vector<unsigned int> ind = { 0, 1, 2 };
	//glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_INT, ind.data());
	m->draw();
...
int main(int argc, char *argv[]) {
	...
	while (running)		// the event loop
	{
		...
		//update();
		draw(window, &a); // call the draw function
		//a.draw();
	}
